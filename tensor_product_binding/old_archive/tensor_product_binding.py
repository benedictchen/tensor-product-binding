...
class TensorProductBinding:
    ...
    def bind(self, filler: Union[str, np.ndarray], role: Union[str, np.ndarray], 
        ...
        ⚡ Advanced Features: This implementation includes ALL Smolensky's methods!
        ...
        if operation == BindingOperation.CIRCULAR_CONVOLUTION:
            # Import circular convolution from shared math utils
            try:
                from ..shared_utils.math_utils import circular_convolution
            except ImportError:
                from shared_utils.math_utils import circular_convolution
            if len(role_vec) != len(filler_vec):
                # Pad vectors to same length for circular convolution
                max_len = max(len(role_vec), len(filler_vec))
                role_padded = np.pad(role_vec, (0, max_len - len(role_vec)), mode='constant')
                filler_padded = np.pad(filler_vec, (0, max_len - len(filler_vec)), mode='constant')
                result = circular_convolution(role_padded, filler_padded)
                return TPBVector(result)
            else:
                result = circular_convolution(role_vec, filler_vec)
                return TPBVector(result)
        elif operation == BindingOperation.HOLOGRAPHIC_REDUCED:
            # Holographic Reduced Representation binding (Plate 1995)
            try:
                from ..shared_utils.math_utils import circular_convolution
            except ImportError:
                from shared_utils.math_utils import circular_convolution
            # HRR binding is circular convolution
            if len(role_vec) != len(filler_vec):
                max_len = max(len(role_vec), len(filler_vec))
                role_padded = np.pad(role_vec, (0, max_len - len(role_vec)), mode='constant')
                filler_padded = np.pad(filler_vec, (0, max_len - len(filler_vec)), mode='constant')
            ...
            # Vector-matrix multiplication binding
            # Use role as matrix selector/transformer, filler as content
            if len(role_vec) == len(filler_vec):
                # Create transformation matrix from role vector
                n = len(role_vec)
                transform_matrix = np.outer(role_vec, np.ones(n)) + np.eye(n) * np.mean(role_vec)
                result = transform_matrix @ filler_vec
                return TPBVector(result)
            else:
                # Fallback to element-wise multiplication if dimensions don't match
                min_len = min(len(role_vec), len(filler_vec))
                result = role_vec[:min_len] * filler_vec[:min_len]
            return TPBVector(result)
        ...
        # Return configured binding result as flattened vector
    ...
    # ======================================================================================
    # 🔧 COMPREHENSIVE IMPLEMENTATIONS - Tensor Product Binding Configuration
    # ======================================================================================
    
    def bind_recursive(self, hierarchical_structure: Dict, depth: int = 3, 
                      binding_strength: float = 1.0) -> np.ndarray:
        """
        Public method for recursive hierarchical binding - COMPLETE IMPLEMENTATION
        
        Implements: "Recursive binding for hierarchical structures (Section 4)"
    ...
    def enable_compositional_binding(self, enable: bool = True):
        """
        Enable compositional binding mode - COMPLETE IMPLEMENTATION
        
        Implements: "Context-dependent binding for ambiguous roles (Section 5)"
    ...
    def configure_binding_mechanism(self, mechanism_type: str = 'basic_outer_product'):
        """
        Configure binding mechanism type - COMPREHENSIVE IMPLEMENTATION
        ...
        # Map string to enum and update config
        mechanism_map = {
            'basic_outer_product': BindingMethod.BASIC_OUTER_PRODUCT,
            'recursive': BindingMethod.RECURSIVE_BINDING,
            'context_dependent': BindingMethod.CONTEXT_DEPENDENT,
            'weighted': BindingMethod.WEIGHTED_BINDING,
            'multi_dimensional': BindingMethod.MULTI_DIMENSIONAL,
            'hybrid': BindingMethod.HYBRID
        }
        
        self.config.binding_method = mechanism_map[mechanism_type]
        self.binding_mechanism = mechanism_type  # Keep for backwards compatibility
    ...
    def configure_unbinding_method(self, method_type: str = 'basic'):
        ...
        Implements: "All optimal unbinding methods from Smolensky 1990"
        ...
        # Map string to enum and update config
        unbinding_map = {
            'basic': UnbindingMethod.BASIC_MULTIPLICATION,
            'least_squares': UnbindingMethod.LEAST_SQUARES,
            'regularized': UnbindingMethod.REGULARIZED,
            'iterative': UnbindingMethod.ITERATIVE,
            'context_sensitive': UnbindingMethod.CONTEXT_SENSITIVE
        }
        
        self.config.unbinding_method = unbinding_map[method_type]
        self.unbinding_method = method_type  # Keep for backwards compatibility
        print(f"✓ Tensor Product unbinding method set to: {method_type}")
        
    def update_config(self, **config_updates) -> None:
        """
        Update configuration parameters dynamically - COMPREHENSIVE USER CONFIGURATION
        
        Allows users to modify any configuration parameter after initialization.
        Provides maximum flexibility for all Smolensky 1990 features.
        
        Args:
            **config_updates: Any TensorBindingConfig parameters to update
        ...
        print(f"✓ Configuration updated with {len(config_updates)} parameters")
        
    def get_configuration_options(self) -> Dict[str, Any]:
        """
        Get all available configuration options - COMPLETE USER VISIBILITY
        
        Returns complete list of all configurable parameters for maximum user control
        over all Smolensky 1990 features and advanced tensor product binding options.
        """
        return {
            'binding_methods': [method.value for method in BindingMethod],
            'unbinding_methods': [method.value for method in UnbindingMethod],
            'binding_operations': [op.value for op in BindingOperation],
            'current_binding_method': self.config.binding_method.value,
            'current_unbinding_method': self.config.unbinding_method.value,
            'configurable_parameters': {
                ...
                'enable_cleanup_memory': self.config.enable_cleanup_memory,
                'enable_role_ambiguity_resolution': self.config.enable_role_ambiguity_resolution,
                'enable_hierarchical_unbinding': self.config.enable_hierarchical_unbinding
            }
        }
        
    def reset_to_defaults(self) -> None:
        """Reset configuration to default values"""
        self.config = TensorBindingConfig()
    ...
    def create_structure(self, bindings: List[Tuple[str, str]], structure_name: str) -> np.ndarray:
        ...
        for role, filler in bindings:
            ...
            binding_result = self.bind(role, filler)
            
            # Convert TPBVector back to matrix form for superposition
            if isinstance(binding_result, TPBVector):
                binding_matrix = binding_result.data.reshape(self.role_dim, self.symbol_dim)
            else:
                binding_matrix = binding_result
            
            # Add to composite (superposition principle)
            composite_tensor += binding_matrix
    ...
    def unbind(self, structure_tensor: np.ndarray, query_role: Union[str, np.ndarray],
        ...
        ⚡ Advanced Unbinding: This implementation includes ALL Smolensky's unbinding methods!
        ✅ Least-squares unbinding for overdetermined systems (Section 3.4)
        ✅ Regularized unbinding to handle noise and interference (Section 7)
        ✅ Iterative unbinding for complex hierarchical structures (Section 4.3)
        ✅ Context-sensitive unbinding for role ambiguity (Section 5.2)
